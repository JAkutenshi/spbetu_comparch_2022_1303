...
extern void func(int *res, int *nums, int k, int n, int *arr, int *min_res);
...
int main() {
  ...
  int *min_res = calloc(k, sizeof(int));
  for (int i = 0; i < k; i++) {
    min_res[i] = INT_MAX;
  }

  func(res, nums, k, n, arr, min_res);
