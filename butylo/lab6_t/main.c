...
extern void processing_intervals(int *result_array, int *source_array,
                                 int *borders_array, int number, int borders_number, int *count_odd);
...
int main() {
    ...
    for (int i = 0; i < intervals; ++i) {
        fprintf(f, "Interval number: %d; Interval border: %d; Count of occurrences: %d; Count of odd digits: %d.\n", i + 1, intervals_array[i], resultArray[i], odd_array[i]);
    }
