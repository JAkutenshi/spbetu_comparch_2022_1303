...
#include <random>
#include <fstream>
extern "C" {void numcount(int* arr, int  NumsRat, int* arrLGrInt, int NInt, int* res, int* resnech); }
int main()
{
    srand(time(NULL));
    int NumRatDat;
    ...
    int* res2 = new int[NInt] {0};
    numcount(gennumbers, NumRatDat, borders, NInt, res, res2);
    std::ofstream fl("res.txt");
    for (int i = 0; i < NInt; i++) {
        std::cout <<"left border "<< borders[i] << " have " << res[i] <<" numbers, and not even is "<<res2[i]<<" numbers" << "\n";
