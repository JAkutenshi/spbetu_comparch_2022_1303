...
extern "C" {void function(int* Array, int len, int* LGrInt, int NInt, int* answer, int* even_num); }
...
int main() {
	...
	int *res_even_numbers = new int[NInt] {0};
	function(n_arr, NumRanDat, int_arr, NInt, res_arr, res_even_numbers);
	...
	file << "\n\nResults:\n";
	for (int i = 0; i < NInt; ++i) {
		std::cout << i + 1 << ". left border:" << int_arr[i] << "  numbers - " << res_arr[i] << " even numbers - " << res_even_numbers[i] << "\n";
		file << i + 1 << ". left border:" << int_arr[i] << "  numbers - " << res_arr[i] << " even numbers - " << res_even_numbers[i] << "\n";
	}
