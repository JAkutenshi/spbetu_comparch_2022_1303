...
ASSUME CS:CODE, SS:AStack ; указываем сегмент, который мы считаем стеком
...
GREETING  DB 'Student from 1303 - Andreeva Elizaveta$'
DATA      ENDS
...
WriteMsg  PROC  NEAR
          mov   AH,9
          int   21h  ; Вызов функции DOS по прерыванию || печатает из DS:dx
          ret ; берет значение из стека и записывает в ip
WriteMsg  ENDP
...
Main      ENDP
CODE      ENDS
          END Main ; начинается программа с main

; если не int, то f1, если int, то f2